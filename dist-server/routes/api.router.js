...
const query_sanitizer_service_1 = require("../services/query-sanitizer.service");
...
const limiter = (0, rate_limiter_middleware_1.slidingWindowRateLimit)({
    windowMs: 60 * 1000,
    maxRequests: 180,
    keyPrefix: 'api-ops'
});
function getInventory() {
    ...
    if (!fs_1.default.existsSync(csvPath))
        return [];
    const lines = fs_1.default.readFileSync(csvPath, 'utf8').split(/\r?\n/).filter(l => l.trim() && l.replace(/,/g, '').trim().length > 0);
    if (lines.length <= 1)
        return [];
...
apiRouter.get('/servers', limiter, (0, rbac_middleware_1.requirePermission)('diagnostics:read'), (_req, res) => {
    res.json(getInventory());
});
apiRouter.get('/servers/:instanceId/databases', limiter, (0, rbac_middleware_1.requirePermission)('diagnostics:read'), async (req, res) => {
    const { instanceId } = req.params;
    const servers = getInventory();
...
apiRouter.get('/catalog', limiter, (0, rbac_middleware_1.requirePermission)('diagnostics:read'), (_req, res) => {
    const catalog = [
        {
            id: 'health',
            label: 'Health Check',
...
apiRouter.get('/query/:categoryId/:queryId', limiter, (0, rbac_middleware_1.requirePermission)('diagnostics:read'), async (req, res) => {
    ...
    try {
        ...
        (0, audit_service_1.writeAuditLog)({
            ...
            clientIp: req.headers['x-forwarded-for'] || req.socket.remoteAddress || '',
            serverId: req.query.server || 'unknown',
            database: req.query.database || 'master',
            queryId,
            durationMs,
            status: 'ERROR',
            rowsReturned: 0,
